...
public partial class EngineeringTools_PartsList_EtagridView_PartsListTSDSubSelect : System.Web.UI.Page
{
    #region Property
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            GridViewDataTable = CommonDB.GetPartListInfoTSDByPackageNameAndParentNumber(PackageName, PartList, true).Tables[0];
    ...
    private void FilterGridDTByECI(string orieci)
    {
        ...
        if (list.Length == 2)
        {
            ...
            if (rows.Length != 0)
            {
                ...
                ECIFilteredGridViewDataTable = dt1;
        ...
        else // Means All
        {
            ...
            ECIFilteredGridViewDataTable = GridViewDataTable;
        }
    }
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        string txtname2 = this.txtSubName2.Text;
        string txtname3 = this.txtSubName3.Text;
        string txtname4 = this.txtSubName4.Text;
        //string txtcode1=
    ...
    private DataTable MakeDataTable()
    {
        ...
        // Get current ordervalue  of gridView
        // string selectedordervalue=
        // int selectindex = 0;

        int srowidvalue = Convert.ToInt16(hiddenfieldgvselectrowid.Value.Split(':')[1]);
        Label selectedordervaluelbl = Gvwpartlist.Rows[srowidvalue].FindControl("lblordervalue") as Label;
        if (selectedordervaluelbl == null)
        {
        ...
        for (int i = 1; i <= 6; i++)
        {
            ...
            if (chk.Checked)
            {
                ...
                else
                {
                    ...
                    if (partentpart == null)
                    {
                        // not found partentpart from six rows
                        ...
                        DataTable filterdt =
                            ECIFilteredGridViewDataTable.Select("ordervalue<='" + selectordervalue + "'",
                                                                "ordervalue DESC").CopyToDataTable();
    ...
    private void GenergicCheckBoxChangedHandler(object sender)
    {
        ...
        GridViewRow selectedrow = GetGridViewRowSelectedRow();
        // check if any radio buttion selected or not 
        if (selectedrow == null)
        {
            chk.Checked = false;
            // display error message
            ErrorMessage = "Please select item in Parts List before entering new items.";
            return;
        }
        ...
        if (i == 1)
        {
            ...
            maxrowlevel = Convert.ToInt16(gslevel) + 1;
        }
        else
        {
            // find top last row 
    ...
    /// </summary>
    /// <returns></returns>
    private GridViewRow GetGridViewRowSelectedRow()
    {
        GridViewRow result = null;
        foreach (GridViewRow row in Gvwpartlist.Rows)
        {
            if (((CheckBox)row.FindControl("rdoselect")).Checked)
            {
                //read the label 
                result = row;
    ...
    protected void Gvwpartlist_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label lblpartno = (Label)e.Row.Cells[2].FindControl("Labelpartno");

                RadioButton control = (RadioButton)e.Row.Cells[0].FindControl("rdoselect");

                string partno = lblpartno.Text.Trim();

                if (CommonTool.RexTSDSubPartList.IsMatch(partno))
                {

                    string rowID = Gvwpartlist.ClientID + "row:" + e.Row.RowIndex;
                    e.Row.Attributes.Add("id", Gvwpartlist.ClientID + "row:" + e.Row.RowIndex);

                    e.Row.Attributes.Add("onclick", "ChangeRowColor(" + "'" + rowID + "'" + ",'" + partno + "')");

                    string
                        //strScript = "uncheckOthersradio(" + ((CheckBox)e.Row.Cells[0].FindControl("rdoselect")).ClientID + ");";

                        strScript = "uncheckOthersradio(" + control.ClientID + ",'" + partno + "')";
